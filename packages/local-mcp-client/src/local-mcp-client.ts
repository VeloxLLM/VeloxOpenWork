...
import type {
  ...
  RemoteMcpAbandonAuthorizationInput,
  RemoteMcpCallToolInput,
  RemoteMcpClient,
  RemoteMcpClientOptions,
  RemoteMcpClock,
  RemoteMcpCompleteAuthorizationInput,
  RemoteMcpConnectInput,
  RemoteMcpConnectResult,
  RemoteMcpConnection,
  RemoteMcpFetch,
  RemoteMcpLifecycle,
  RemoteMcpListResourcesInput,
  RemoteMcpListResourceTemplatesInput,
  RemoteMcpListToolsInput,
  RemoteMcpOperationPhase,
  RemoteMcpRequestPhase,
  RemoteMcpReadResourceInput,
} from "./contracts.js"
import { RemoteMcpClientError, RemoteMcpLifecycleDeadlineError, RemoteMcpToolResultError } from "./errors.js"
import { RemoteMcpOAuthProvider } from "./oauth-provider.js"
import { createRemoteMcpRequestObserver, type RemoteMcpRequestObserver } from "./request-observer.js"
import { createRemoteMcpTokenResponseCompat } from "./token-response-compat.js"
import { collectRemoteMcpTools } from "./tool-catalog.js"
import {
  assertRemoteMcpResourceResult,
  collectRemoteMcpResources,
  collectRemoteMcpResourceTemplates,
  REMOTE_MCP_RESOURCE_URI_LIMIT_BYTES,
} from "./resource-catalog.js"
import { assertRemoteMcpToolArguments } from "./tool-input.js"
...
const resourceUriSchema = z.string().trim().min(1).max(REMOTE_MCP_RESOURCE_URI_LIMIT_BYTES).refine(
  (value) => Buffer.byteLength(value, "utf8") <= REMOTE_MCP_RESOURCE_URI_LIMIT_BYTES,
...
export const REMOTE_MCP_PROTOCOL_VERSION_FALLBACK = "2025-06-18"
...
type Session = {
  ...
  serverUrl: URL
  oauthProvider?: RemoteMcpOAuthProvider
  observer: RemoteMcpRequestObserver
...
function requestInit(authorization: RemoteMcpAuthorization): RequestInit | undefined {
  if (authorization.type !== "api-key") return undefined
  return { headers: { authorization: `Bearer ${authorization.token}` } }
}

function validateConnection(connection: RemoteMcpConnection): URL {
  ...
  if (url.protocol !== "https:" && url.protocol !== "http:") {
    throw new Error("An remote MCP server URL must use HTTP or HTTPS.")
  }
  if (url.username || url.password) {
    throw new Error("An remote MCP server URL cannot contain embedded credentials.")
  }
  if (url.hash) throw new Error("An remote MCP server URL cannot contain a fragment.")
  return url
}
...
function validateRedirectUri(redirectUri: string): string {
  ...
  if (url.protocol !== "https:" && url.protocol !== "http:") {
    throw new Error("An remote MCP OAuth redirect URI must use HTTP or HTTPS.")
  }
  if (url.username || url.password || url.hash) {
    throw new Error("An remote MCP OAuth redirect URI cannot contain credentials or a fragment.")
  }
  return parsed
}
...
function configurationValue<T>(parse: () => T): T {
  try {
    ...
    throw new RemoteMcpClientError({
...
export function createRemoteMcpClient(options: RemoteMcpClientOptions): RemoteMcpClient {
  ...
  const clock: RemoteMcpClock = options.clock ?? { now: () => Date.now() }
  const configuredFetch: RemoteMcpFetch = options.fetch

  function emitDiagnostic(event: Parameters<NonNullable<RemoteMcpClientOptions["diagnosticSink"]>>[0]): void {
    try {
  ...
  function failureRequestPhase(observer: RemoteMcpRequestObserver) {
    return observer.lastFailedRequestPhase() ?? observer.lastRequestPhase()
  }

  function isMcpResourceRequest(phase: RemoteMcpRequestPhase): boolean {
  ...
  async function invalidateTerminallyRejectedCredential(
    session: Session,
    input: { connectionId: string; operationPhase: RemoteMcpOperationPhase },
  ): Promise<void> {
  ...
  function createSession(input: {
    ...
    const timeout = setTimeout(() => {
      controller.abort(new RemoteMcpLifecycleDeadlineError(input.operationPhase))
    }, remaining)
    controller.signal.addEventListener("abort", () => clearTimeout(timeout), { once: true })

    const compatibleFetch = createRemoteMcpTokenResponseCompat({
    ...
    const observer = createRemoteMcpRequestObserver({
      connectionId: input.connection.id,
      operationPhase: input.operationPhase,
  ...
  async function connectWithProtocolVersionFallback(input: {
    ...
    emitDiagnostic({
      ...
      outcome: "started",
      protocolVersionFallback: REMOTE_MCP_PROTOCOL_VERSION_FALLBACK,
    })
    input.session.client = new Client(
      { name: clientName, version: clientVersion },
      {
        ...
        protocolVersion: REMOTE_MCP_PROTOCOL_VERSION_FALLBACK,
      },
    )
  ...
  async function runOperation<T>(input: {
    ...
    try {
      ...
      const wrapped = error instanceof RemoteMcpClientError
        ? error
        : new RemoteMcpClientError({
            operationPhase: input.operationPhase,
  ...
  return {
    async connect(input: RemoteMcpConnectInput): Promise<RemoteMcpConnectResult> {
    ...
    async completeAuthorization(input: RemoteMcpCompleteAuthorizationInput): Promise<void> {
    ...
    async abandonAuthorization(input: RemoteMcpAbandonAuthorizationInput): Promise<void> {
      const authorizationId = configurationValue(() => authorizationIdSchema.parse(input.authorizationId))
    ...
    async listTools(input: RemoteMcpListToolsInput) {
      return runConnectedOperation({
        connection: input.connection,
        redirectUri: input.redirectUri,
        operationPhase: "tool-discovery",
        operation: async (session) => {
          return collectRemoteMcpTools({
    ...
    async callToolRaw(input: RemoteMcpCallToolInput) {
      const toolName = configurationValue(() => toolNameSchema.parse(input.toolName))
      configurationValue(() => assertRemoteMcpToolArguments(input.arguments))
      return runConnectedOperation({
        connection: input.connection,
        redirectUri: input.redirectUri,
    ...
    async callTool(input: RemoteMcpCallToolInput) {
      const toolName = configurationValue(() => toolNameSchema.parse(input.toolName))
      configurationValue(() => assertRemoteMcpToolArguments(input.arguments))
      return runConnectedOperation({
        connection: input.connection,
        ...
        operation: async (session) => {
          ...
          if ("isError" in result && result.isError) throw new RemoteMcpToolResultError(result)
          return result
        },
      })
    },

    async listResources(input: RemoteMcpListResourcesInput) {
      return runConnectedOperation({
        connection: input.connection,
        redirectUri: input.redirectUri,
        operationPhase: "resource-discovery",
        operation: (session) => collectRemoteMcpResources({
          requestOptions: session.requestOptions,
    ...
    async readResource(input: RemoteMcpReadResourceInput) {
      ...
      return runConnectedOperation({
        ...
        operation: async (session) => {
          ...
          assertRemoteMcpResourceResult(result)
          return result
        },
      })
    },

    async listResourceTemplates(input: RemoteMcpListResourceTemplatesInput) {
      return runConnectedOperation({
        connection: input.connection,
        redirectUri: input.redirectUri,
        operationPhase: "resource-discovery",
        operation: (session) => collectRemoteMcpResourceTemplates({
          requestOptions: session.requestOptions,
    ...
    async describeServer(input: RemoteMcpListResourcesInput) {
      return runConnectedOperation({
        connection: input.connection,
        redirectUri: input.redirectUri,
