...
import type {
  RemoteMcpClock,
  RemoteMcpLifecycle,
  RemoteMcpOAuthAuthorizationHandle,
  RemoteMcpOAuthClientRegistration,
  RemoteMcpOAuthCredential,
  RemoteMcpOAuthConfiguration,
  RemoteMcpOAuthPersistence,
  RemoteMcpPersistenceContext,
} from "./contracts.js"
import { RemoteMcpOAuthContractError } from "./errors.js"
...
export class RemoteMcpOAuthProvider implements OAuthClientProvider {
  ...
  private readonly persistence: RemoteMcpOAuthPersistence
  private readonly flow: OAuthFlowContext
  private readonly clientName: string
  private readonly clock: RemoteMcpClock
  private readonly lifecycle: RemoteMcpLifecycle
  private readonly authorizationTransactionTtlMs: number
  private readonly expirationSkewMs: number
  private readonly applicationType: RemoteMcpOAuthConfiguration["applicationType"]
  ...
  private loadedClient: RemoteMcpOAuthClientRegistration | undefined
  private loadedCredential: RemoteMcpOAuthCredential | undefined
  private loadedDiscovery: OAuthDiscoveryState | undefined
  private authorizationHandle: RemoteMcpOAuthAuthorizationHandle | undefined
  ...
  constructor(input: {
    ...
    oauthConfiguration?: RemoteMcpOAuthConfiguration
  }) {
  ...
  private context(): RemoteMcpPersistenceContext {
    const now = this.clock.now()
    if (this.lifecycle.signal.aborted || now >= this.lifecycle.expiresAt) {
      throw new RemoteMcpOAuthContractError(
        "MCP_LIFECYCLE_DEADLINE",
        "The remote MCP lifecycle expired before OAuth persistence could continue.",
      )
    }
    return {
