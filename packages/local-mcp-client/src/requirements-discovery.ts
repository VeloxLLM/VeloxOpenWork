...
import type {
  DiscoverRemoteMcpConnectionRequirementsInput,
  RemoteMcpAuthorizationServerRequirement,
  RemoteMcpConnectionRequirements,
  RemoteMcpFetch,
} from "./contracts.js"
...
export function selectRecoverableAuthorizationServerIssuer(input: {
  ...
  requirements: Pick<RemoteMcpConnectionRequirements, "authentication" | "warnings">
}): string | undefined {
  if (
...
): RemoteMcpAuthorizationServerRequirement | null {
...
}): RemoteMcpConnectionRequirements["manualRequirements"] {
  const requirements: RemoteMcpConnectionRequirements["manualRequirements"] = [
    {
...
export async function discoverConnectionRequirements(
  input: DiscoverRemoteMcpConnectionRequirementsInput,
): Promise<RemoteMcpConnectionRequirements> {
  ...
  if (serverUrl.username || serverUrl.password || serverUrl.hash) {
    throw new Error("An remote MCP server URL cannot contain credentials or a fragment.")
  }
  ...
  const warnings: RemoteMcpConnectionRequirements["warnings"] = []
  const tools: NonNullable<RemoteMcpConnectionRequirements["tools"]["items"]> = []
  let initialize: RemoteMcpConnectionRequirements["server"]["initialize"] = "failed"
  ...
  const authorizationServers: RemoteMcpAuthorizationServerRequirement[] = []
  ...
  const availableRegistrationMethods: RemoteMcpConnectionRequirements["authentication"]["availableRegistrationMethods"] = ["pre_registered"]
  ...
  let recommendedRegistrationMethod: RemoteMcpConnectionRequirements["authentication"]["recommendedRegistrationMethod"] = "pre_registered"
  ...
  const status: RemoteMcpConnectionRequirements["status"] = initialize === "succeeded"
    ? "ready"
