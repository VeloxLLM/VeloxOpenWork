...
import type { RemoteMcpOperationPhase, RemoteMcpRequestPhase } from "./contracts.js"
...
const errorCodeByPhase: Record<RemoteMcpOperationPhase, RemoteMcpErrorCode> = {
...
const phaseLabel: Record<RemoteMcpOperationPhase, string> = {
...
const requestPhaseLabel: Record<RemoteMcpRequestPhase, string> = {
...
export class RemoteMcpClientError extends Error {
  ...
  readonly operationPhase: RemoteMcpOperationPhase
  readonly requestPhase: RemoteMcpRequestPhase | null
  ...
  constructor(input: {
    ...
    super(`Local MCP failed during ${phaseLabel[input.operationPhase]}${request}.`, { cause: input.cause })
 ...
 */
export const REMOTE_MCP_LIFECYCLE_DEADLINE_DATA_KEY = "remoteMcpLifecycleDeadline"

export class RemoteMcpLifecycleDeadlineError extends McpError {
  readonly operationPhase: RemoteMcpOperationPhase

  constructor(operationPhase: RemoteMcpOperationPhase) {
    super(ErrorCode.RequestTimeout, `Local MCP ${operationPhase} exceeded its lifecycle deadline.`, {
      [REMOTE_MCP_LIFECYCLE_DEADLINE_DATA_KEY]: true,
      operationPhase,
    })
    this.name = "RemoteMcpLifecycleDeadlineError"
    this.operationPhase = operationPhase
  }
}

/** Walks an error chain for the deadline marker set by this client. */
export function isRemoteMcpLifecycleDeadline(value: unknown): boolean {
  let current: unknown = value
  for (let depth = 0; depth < 5 && current; depth += 1) {
    if (current instanceof RemoteMcpLifecycleDeadlineError) return true
    if (typeof current === "object" && current !== null && "data" in current) {
      const data: unknown = current.data
      if (
        typeof data === "object" && data !== null
        && REMOTE_MCP_LIFECYCLE_DEADLINE_DATA_KEY in data
        && data[REMOTE_MCP_LIFECYCLE_DEADLINE_DATA_KEY] === true
      ) {
        return true
      }
    }
...
export type RemoteMcpOAuthContractErrorCode =
...
export class RemoteMcpOAuthContractError extends Error {
  readonly code: RemoteMcpOAuthContractErrorCode

  constructor(code: RemoteMcpOAuthContractErrorCode, message: string) {
    super(message)
    this.name = "RemoteMcpOAuthContractError"
    this.code = code
  }
}

export class RemoteMcpToolResultError extends Error {
  ...
  constructor(result?: unknown) {
    ...
    this.name = "RemoteMcpToolResultError"
...
export type RemoteMcpToolInputErrorCode =
  | "MCP_TOOL_ARGUMENT_SIZE_LIMIT"
  | "MCP_TOOL_ARGUMENT_DEPTH_LIMIT"
  | "MCP_TOOL_ARGUMENT_CYCLE"
  | "MCP_TOOL_ARGUMENT_INVALID_JSON"

export class RemoteMcpToolInputError extends Error {
  readonly code: RemoteMcpToolInputErrorCode

  constructor(code: RemoteMcpToolInputErrorCode) {
    super("The MCP tool arguments do not satisfy the bounded JSON input contract.")
    this.name = "RemoteMcpToolInputError"
    this.code = code
  }
}

export type RemoteMcpCatalogErrorCode =
...
export class RemoteMcpCatalogError extends Error {
  readonly code: RemoteMcpCatalogErrorCode

  constructor(code: RemoteMcpCatalogErrorCode) {
    super("The MCP catalog or resource exceeded an enterprise client contract limit.")
    this.name = "RemoteMcpCatalogError"
    this.code = code
  }
}
