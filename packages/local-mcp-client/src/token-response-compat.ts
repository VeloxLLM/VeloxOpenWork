import type { RemoteMcpFetch, RemoteMcpRequestPhase } from "./contracts.js"
import { classifyRemoteMcpRequest } from "./request-observer.js"
import {
  REMOTE_MCP_RESPONSE_BODY_EXCERPT_CHARS,
...
type TokenRequestPhase = Extract<RemoteMcpRequestPhase, "oauth-token-exchange" | "oauth-token-refresh">
...
export type RemoteMcpTokenResponseTranslation = {
...
function isTokenRequestPhase(phase: RemoteMcpRequestPhase): phase is TokenRequestPhase {
...
function structuredTokenResponseExcerpt(response: Record<string, unknown>): string {
  ...
  return JSON.stringify(allowed).slice(0, REMOTE_MCP_RESPONSE_BODY_EXCERPT_CHARS)
}
...
function notifyTranslation(
  callback: ((translation: RemoteMcpTokenResponseTranslation) => void) | undefined,
  translation: RemoteMcpTokenResponseTranslation,
): void {
  try {
...
export function createRemoteMcpTokenResponseCompat(input: {
  fetch: RemoteMcpFetch
  onTranslation?: (translation: RemoteMcpTokenResponseTranslation) => void
}): RemoteMcpFetch {
  return async (rawUrl, init) => {
    const response = await input.fetch(rawUrl, init)
    const requestPhase = classifyRemoteMcpRequest(
