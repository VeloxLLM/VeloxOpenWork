...
import {
  ...
  RemoteMcpCatalogError,
  RemoteMcpClientError,
  RemoteMcpToolResultError,
  RemoteMcpToolInputError,
  RemoteMcpOAuthContractError,
...
import { RemoteMcpOAuthProvider } from "../src/oauth-provider.js"
import { createRemoteMcpRequestObserver } from "../src/request-observer.js"
import { collectRemoteMcpTools } from "../src/tool-catalog.js"
import {
...
function mockMcpFetch(options: MockMcpOptions = {}): RemoteMcpFetch {
  ...
  if (rpc.method === "initialize") {
    sendJson(response, 200, {
      ...
      result: {
        protocolVersion: "2025-06-18",
        capabilities: { tools: {} },
        serverInfo: { name: "oauth-remote-mcp-test", version: "1.0.0" },
      },
    })
    return
  }
      ...
      if (rpc.method === "initialize") {
        sendJson(response, 200, {
          ...
          result: {
            protocolVersion: "2025-06-18",
            capabilities: { tools: {} },
            serverInfo: { name: "progress-remote-mcp-test", version: "1.0.0" },
          },
        })
        return
      }
...
function noAuthConnection(): RemoteMcpConnection {
  return {
    id: "connection-1",
...
describe("remote MCP client", () => {
  ...
  it("preserves tools, resources, UI metadata, and tool results over MCP Streamable HTTP", async () => {
    const events: RemoteMcpDiagnosticEvent[] = []
    ...
    assert.deepEqual(descriptor.serverInfo, { name: "remote-mcp-test", version: "1.0.0" })
  ...
  it("sends Den's API key as a bearer credential", async () => {
    const client = createRemoteMcpClient({ fetch: mockMcpFetch({ expectedApiKey: "secret-test-key" }) })
  ...
  it("honors an injected lifecycle longer than the per-request timeout while progress streams", async () => {
    const server = await startProgressMcpServer({ resultDelayMs: 140, progressIntervalMs: 20 })
    try {
      const client = createRemoteMcpClient({
        fetch,
        operationTimeoutMs: 80,
        lifecycle: { expiresAt: Date.now() + 350, signal: new AbortController().signal },
      })
      const result = await client.callTool({
        connection: {
  ...
  it("still stops a progressing provider at the injected absolute lifecycle", async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        (error: unknown) => error instanceof RemoteMcpClientError
  ...
  it("treats an MCP isError tool result as a failed operation", async () => {
    const client = createRemoteMcpClient({ fetch: mockMcpFetch({ toolError: true }) })
    await assert.rejects(
      client.callTool({
        connection: noAuthConnection(),
        redirectUri: "https://den.example.test/callback",
      ...
      (error: unknown) => {
        ...
        assert.ok(error.cause instanceof RemoteMcpToolResultError)
        return true
      },
    )
  })
  ...
  it("rejects oversized or cyclic tool arguments before opening a provider connection", async () => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof RemoteMcpClientError
        && error.cause instanceof RemoteMcpToolInputError
...
class MemoryOAuthPersistence implements RemoteMcpOAuthPersistence {
  registration: RemoteMcpOAuthClientRegistration | undefined
  credential: RemoteMcpOAuthCredential | undefined
  authorizationRecords = new Map<string, { handle: RemoteMcpOAuthAuthorizationHandle; codeVerifier: string }>()
  invalidationCount = 0
  revision = 0
  ...
  readonly credentials = {
    ...
    save: async (input: {
      ...
      if (input.source === "authorization-code") {
        ...
        throw new RemoteMcpOAuthContractError(
...
function oauthRuntimeConnection(id: string, persistence: RemoteMcpOAuthPersistence): RemoteMcpConnection {
  return {
    id,
    serverUrl: "https://mcp.example.test/mcp",
...
describe("remote MCP OAuth persistence contract", () => {
  ...
  it("validates authorization-response issuers exactly before token exchange", () => {
    ...
    assert.throws(() => validateMcpAuthorizationResponseIssuer({
      ...
      discoveryState,
      responseIssuer: "https://identity.example.test/tenant/",
    }), (error: unknown) => error instanceof RemoteMcpOAuthContractError
  ...
  it("completes discovery, dynamic registration, PKCE exchange, and authenticated MCP initialization", async () => {
    ...
    try {
      const persistence = new MemoryOAuthPersistence()
      const events: RemoteMcpDiagnosticEvent[] = []
      const client = createRemoteMcpClient({ fetch, diagnosticSink: (event) => events.push(event) })
  ...
  it("falls back to advertised scopes when the challenge and requested scopes are empty", async () => {
    ...
    try {
      const persistence = new MemoryOAuthPersistence()
      const client = createRemoteMcpClient({ fetch })
      const connection: RemoteMcpConnection = {
  ...
  it("starts OAuth when initialize is public but tool discovery requires authorization", async () => {
    ...
    const metadataUrl = "https://den.example.test/oauth/client-metadata.json"
    const fetch: RemoteMcpFetch = async (url, init) => {
  ...
  it("invalidates exchanged tokens when callback validation cannot initialize MCP", async () => {
    ...
    try {
      const persistence = new MemoryOAuthPersistence()
      const client = createRemoteMcpClient({ fetch, operationTimeoutMs: 5_000 })
      const connection: RemoteMcpConnection = {
...
describe("remote MCP catalog contract", () => {
  it("collects resources without changing standard UI metadata", async () => {
    const resources = await collectRemoteMcpResources({
      requestOptions: {},
  ...
  it("rejects duplicate resource URIs and oversized resource results", async () => {
    await assert.rejects(collectRemoteMcpResources({
      requestOptions: {},
      listPage: async (cursor) => cursor
        ? { resources: [{ name: "Again", uri: "ui://duplicate" }] }
        : { resources: [{ name: "First", uri: "ui://duplicate" }], nextCursor: "again" },
    }), (error: unknown) => error instanceof RemoteMcpCatalogError
      && error.code === "MCP_CATALOG_DUPLICATE_RESOURCE")
    assert.throws(
      () => assertRemoteMcpResourceResult({ contents: [{ text: "x".repeat(2 * 1024 * 1024) }] }),
      (error: unknown) => error instanceof RemoteMcpCatalogError
  ...
  it("collects a bounded paginated tool catalog", async () => {
    const tools = await collectRemoteMcpTools({
      requestOptions: {},
      listPage: async (cursor) => cursor
        ? {
  ...
  it("rejects duplicate tools across catalog pages", async () => {
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.ok(error instanceof RemoteMcpCatalogError)
