...
import {
  ...
  createRemoteMcpTokenResponseCompat,
  REMOTE_MCP_PROTOCOL_VERSION_FALLBACK,
...
describe("Slack-style MCP compatibility", () => {
  ...
  it("turns an HTTP-200 ok:false token exchange into a typed OAuth error", async () => {
    const translations: RemoteMcpDiagnosticEvent[] = []
  ...
  it("passes standard token responses through as the original Response", async () => {
    ...
    const compatibleFetch = createRemoteMcpTokenResponseCompat({ fetch: async () => original })
  ...
  it("retries initialize exactly once with the fallback protocol version", async () => {
    ...
    assert.equal(protocolVersions[1], REMOTE_MCP_PROTOCOL_VERSION_FALLBACK)
    assert.equal(events.filter((event) => event.kind !== "credential-invalidation"
      && event.protocolVersionFallback === REMOTE_MCP_PROTOCOL_VERSION_FALLBACK).length, 1)
  })
