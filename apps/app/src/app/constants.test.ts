...
describe("OpenWork extension catalog platform filter", () => {
  ...
  test("hides desktop-only extensions in web", () => {
    expect(filteredIds("web")).toEqual(["openwork-voice"]);
  });

  test("keeps OpenWork Browser desktop-only and Computer Use mac-only", () => {
    expect(filteredIds("darwin")).toEqual(["openwork-browser", "computer-use", "openwork-voice"]);
    expect(filteredIds("linux")).toEqual(["openwork-browser", "openwork-voice"]);
  });
});
