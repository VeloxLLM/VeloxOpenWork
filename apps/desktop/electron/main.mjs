...
<html>
  ...
  <div class="title">Stopping VeloxOpenWork services</div>
...
function assertOpenworkServerReady(info) {
  if (!info?.running) {
    throw new Error("VeloxOpenWork server did not stay running after startup.");
  }
  if (!info.baseUrl) {
    throw new Error("VeloxOpenWork server did not report a base URL after startup.");
  }
  if (!info.ownerToken && !info.clientToken) {
    throw new Error("VeloxOpenWork server did not report an access token after startup.");
  }
  return info;
}
...
async function readSecureSecretMap(filePath) {
  ...
  try {
    ...
    const plain = safeStorage.decryptString(Buffer.from(String(payload.data ?? ""), "base64"));
...
if (!app.requestSingleInstanceLock()) {
  if (isDevMode && !app.isPackaged) {
    console.error(`[veloxopenwork] Another VeloxOpenWork dev instance already holds this profile directory:
